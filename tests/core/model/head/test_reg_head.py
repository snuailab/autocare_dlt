...
from autocare_dlt.core.model.head import RegressionHead as Head


class TestRegHead(unittest.TestCase):
    def setUp(self):
        self.num_classes = 1
        self.batch_size = 2
        self.dummy_cfg = dict(in_channels=512, num_classes=self.num_classes)
        self.dummy_input = torch.rand(self.batch_size, 512)
        self.dummy_labels = torch.FloatTensor([[1]] * self.batch_size)
