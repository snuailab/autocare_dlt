...
class TestPoseTrainer(unittest.TestCase):
        ...
        args = Box(
            {
                ...
                "model_cfg":  "tests/assets/pose/configs/HumanPoseNet.json",
                "data_cfg": "tests/assets/pose/configs/pose_small.json",
                "gpus": "0",
                "num_gpus": 1,
                "world_size": 1,
                "ckpt": None,
    ...
    def test_build_trainer(self):
        ...
        self.trainer = PoseTrainer(self.model, self.datasets, self.dummy_cfg)
        self.assertTrue(os.path.exists(self.trainer.output_path))
        with self.assertRaises(ValueError):
            cfg = copy.deepcopy(self.dummy_cfg)
            cfg.pop("optim")
            trainer = PoseTrainer(self.model, self.datasets, cfg)
